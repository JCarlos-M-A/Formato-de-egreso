...
export function genFPortadaEmpastadoDocummentoTitulacion(accion, alumno, contacto, proyecto) {
    ...
    doc.setFontSize(16);
    doc.text('"'+proyecto.nombreProyecto.toUpperCase()+'"', doc.internal.pageSize.width / 2, 90, "center");

    doc.setFontSize(14);
    doc.text(proyecto.producto.toUpperCase(), doc.internal.pageSize.width/2, 110,'center');
    
    doc.text('QUE PARA OBTENER EL TÍTULO DE:', doc.internal.pageSize.width/2, 130,'center');
    doc.text(alumno.carrera.toUpperCase(), doc.internal.pageSize.width/2, 145,'center');

    doc.text('PRESENTA:', doc.internal.pageSize.width/2, 170,'center');
    doc.setFontSize(16);
    doc.text(alumno.nombre.toUpperCase(), doc.internal.pageSize.width/2, 180,'center');

    doc.setFontSize(14);
    doc.text('ASESOR ACADEMICO:', doc.internal.pageSize.width/2, 200,'center');
    doc.setFontSize(16);
    doc.text(proyecto.acesorInterno.toUpperCase(), doc.internal.pageSize.width/2, 210,'center');

    doc.setFontSize(14);
    let date = new Date();
    doc.text(contacto.localidad + ", " + contacto.estado + " a " + date.getDate() + "/" + date.getMonth() + "/" + date.getFullYear(), doc.internal.pageSize.width-doc.getTextWidth(contacto.localidad + ", " + contacto.estado + " a " + date.getDate() + "/" + date.getMonth() + "/" + date.getFullYear())-20, 235);
